...
class TwoFishCrypter : ICipher {
    ...
    override fun encrypt(input: String, password: String?, publicKey: PublicKey?): String {
        ...
        val keySpec = SecretKeySpec(cryptoResourceGenerator.deriveKey(password!!, salt, CryptoConstants.TWOFISH_KEY_SIZE), AlgorithmType.TWOFISH.name)
        val ivSpec = IvParameterSpec(cryptoResourceGenerator.generateIV(CryptoConstants.TWOFISH_IV_SIZE))

        val cipher = Cipher.getInstance(CryptoConstants.TWOFISH_TRANSFORMATION, "BC")  // Using BouncyCastle
    ...
    override fun decrypt(input: String, password: String?, privateKey: PrivateKey?): String {
        ...
        val salt = ByteArray(16) // Assuming the salt size is 16 bytes
        System.arraycopy(decodedInput, 0, salt, 0, salt.size)
        val iv = ByteArray(CryptoConstants.TWOFISH_IV_SIZE)
